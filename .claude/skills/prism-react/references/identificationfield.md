...
Takes in a value used for identifying a resident/citizen and formats the field. Currently only supports United States Social Security Number and Canadian Social Insurance Number.

## Exported Constants
* `IdentificationFieldSize`
...
| Default | `US` |
| Description | Specify to format either US Social Security Number or Canadian Social Insurance Number. |
...
| | |
|-----------|------------|
| Type | `(value: V, formattedValue: string, e: React.ChangeEvent<T>) => void`<br>*`V` = `string`, `T` = `HTMLInputElement`* |
| Default | `null` |
| Description | Callback for when the component's value is changed; called with the raw digits as first argument and formatted string (e.g. `"XXX-XX-XXXX"`) as second. |
...
| | |
|-----------|------------|
| Type | `**IdentificationFieldSize**` |
| Default | `IdentificationFieldSize.Medium` |
...
### IdentificationField - default use
```javascript
import { IdentificationField } from '@doordash/prism-react'
…
<IdentificationField
  country="US"
  value={this.state.ssn}
  onChange={value => this.setState({ ssn: value })}
