...
### EmailField - default use
```typescript
import { EmailField } from '@doordash/prism-react'
...
/>
```
### EmailField - with validation and clear button
```typescript
import { EmailField } from '@doordash/prism-react'
...
-   Have solid error handling for emails; don't be too restrictive.
-   Provide a useful error message for invalid emails; call out _how_ the value is invalid, not just that it is.