...
import { Search, Filter, Sparkles, MessageSquare, Layers, BookOpen, Send, ChevronLeft, ChevronUp, LayoutDashboard, Code2, FileText } from 'lucide-react';
...
export function HomePage() {
  ...
  // Discovery section shows all artifacts regardless of top search box
  ...
  const submitPrompt = () => {
    ...
    setTimeout(() => {
      setMessages([
        ...
        {
          role: 'assistant',
          content: 'analysis' // Special content marker for rendering the detailed analysis
        }
      ]);
      setIsLoading(false);
    }, 2000);
  ...
  return (
    <div className="h-full bg-white overflow-hidden relative flex flex-col">
      {/* Discovery Section - slides down when chat is centered */}
      <motion.div
        animate={{
          y: isChatCentered ? '100%' : 0,
        }}
        transition={{ duration: 0.5, ease: [0.4, 0, 0.2, 1] }}
        className="p-8 flex-1 overflow-auto"
      >
        <div className="max-w-7xl mx-auto">
          {/* Greeting - hidden in normal view since it's shown above the chatbox */}
      ...
      {/* Chat Interface - changes layout when messages exist */}
      {isChatCentered && hasMessages ? (
        // Full screen chat mode with input at bottom
                    ...
                    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                      ...
                      <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-gray-600 text-sm">Completed in 3 minutes and 4 seconds</span>
                        </div>
                      ...
                      <div className="space-y-6">
                        <h2 className="text-xl text-gray-900 font-semibold">DashPass growth deep-dive (last 60 days)</h2>
                        
                        {/* Image 1 - Metric definitions */}
                        <div>
                          <img src={analysisImage1} alt="Metric definitions and methodology" className="w-full rounded-lg border border-gray-200" />
                        </div>

                        {/* Image 2 - Executive summary table */}
                        <div>
                          <img src={analysisImage2} alt="Executive summary by country" className="w-full rounded-lg border border-gray-200" />
                        </div>

                        {/* Image 3 - Key takeaways and trend highlights */}
                        <div>
                          <img src={analysisImage3} alt="Key takeaways and trend highlights" className="w-full rounded-lg border border-gray-200" />
                        </div>

                        {/* Image 4 - Recommended next cuts */}
                        <div>
                          <img src={analysisImage4} alt="Recommended next cuts for analysis" className="w-full rounded-lg border border-gray-200" />
                        </div>

                        {/* Chart Image */}
                        <div className="relative group">
                          <img src={chartImage} alt="Chart showing growth trends" className="w-full rounded-lg border border-gray-200" />
        ...
        <div className="absolute left-0 right-0 pointer-events-none px-8" style={{ top: '32px' }}>
          <motion.div
            animate={{
              y: isChatCentered && !hasMessages ? 'calc(50vh - 200px)' : 0,
            }}
            transition={{ duration: 0.5, ease: [0.4, 0, 0.2, 1] }}
            className="pointer-events-auto max-w-7xl mx-auto"
          >
            {/* Back Button - only visible when centered */}
            <AnimatePresence>
              {isChatCentered && !hasMessages && (
                <motion.button
            ...
            {/* Chat Box */}
            <div
              className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 cursor-pointer transition-shadow hover:shadow-md"
            ...
            >
              ...
              {/* Mode and Purpose Selectors */}
              <div className="flex items-center justify-between">
                {/* Mode Selector: Chat, Hybrid, Notebook */}
                <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                ...
                {/* Purpose Selector: Analysis, Exploration, Reporting */}
                <motion.div
                  animate={{
                ...
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setAgentPurpose('analysis');
                    }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                      agentPurpose === 'analysis'
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setAgentPurpose('exploration');
                    }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                      agentPurpose === 'exploration'
                        ? 'bg-gray-900 text-white border-gray-900'
                        : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                    }`}
                  >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setAgentPurpose('reporting');
                    }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                      agentPurpose === 'reporting'
                        ? 'bg-gray-900 text-white border-gray-900'
                        : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                    }`}
                  >
            ...
            {/* Empty state message when centered */}
            <AnimatePresence>
              {isChatCentered && !hasMessages && (
                <motion.div
