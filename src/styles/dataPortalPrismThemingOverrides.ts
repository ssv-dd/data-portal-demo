import type { ThemingOverridesType } from '@doordash/prism-react'

/**
 * Prism dark: InternalTools dark + partials bridged from `--app-*` (see dataPortalAppSurface).
 * When `useStockPrismDark` is true, use DefaultThemeCollection at call site and return {} here.
 * Ported from Data-Portal-AI-Native/src/styles/prism-theming-overrides.ts
 */
...
/**
 * @param useStockPrism When true, uses Prism defaults only (no `--app-*` bridge).
 */
export function createDataPortalPrismThemingOverrides(useStockPrism: boolean): ThemingOverridesType {
  return (_tokens, _colorMode) => {
    if (useStockPrism) {
      return {}
    }
